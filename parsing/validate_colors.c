...
bool validate_colors(char *file_content, t_main_data *main_data)
{
    ...
    file_content = set_color_value(file_content, &layer->red);
    if (!file_content) return (false);

    file_content = set_color_value(file_content, &layer->green);
    if (!file_content) return (false);

    file_content = set_color_value(file_content, &layer->blue);
    if (!file_content) return (false);
